...
NeuralNetwork* createChild(NeuralNetwork*, NeuralNetwork*);
int gameDone(char**);
void drawBoard(char**);
...
int main() {
	...
	while(generation < 27000) {
		...
		for(int i = 0; i < POPULATION; i++) { //Create child from random parent, mutate, and add it to next generation
			int parent1 = (int)(((double)rand() / RAND_MAX) * sum);
			int parent2 = (int)(((double)rand() / RAND_MAX) * sum);
			while(parent1 == sum) parent1 = (int)(((double)rand() / RAND_MAX) * sum); //There is a chance parent can go out of bounds
			while(parent2 == sum) parent2 = (int)(((double)rand() / RAND_MAX) * sum);
			NeuralNetwork* child = createChild(nns[population_selection[parent1]], nns[population_selection[parent2]]);
...
NeuralNetwork* createChild(NeuralNetwork* p1, NeuralNetwork* p2) {
	NeuralNetwork* child = p1->copy_network();
	for(int i = 0; i < 3; i++) { //indicies of weights/biases
		Matrix* child_weight = child->getWeight(i);
		Matrix* child_bias = child->getBias(i);
		Matrix* parent_weight = p2->getWeight(i);
		Matrix* parent_bias = p2->getBias(i);

		for(int r = 0; r < child_weight->getRows(); r++) {
			for(int c = 0; c < child_weight->getCols(); c++) {
				if(((double)rand() / RAND_MAX) > CROSSOVER_RATE) {
					child_weight->setNum(parent_weight->getNum(r, c), r, c);
				}
			}
		}
		for(int r = 0; r < child_bias->getRows(); r++) {
			for(int c = 0; c < child_bias->getCols(); c++) {
				if(((double)rand() / RAND_MAX) > CROSSOVER_RATE) {
					child_bias->setNum(parent_bias->getNum(r, c), r, c);
				}
			}
		}
		//child->setWeight(i, child_weight);
		//child->setBias(i, child_bias);
	}
	return child;
}
